...
print_char (48+ans);
print_char (48+x-tosub) in
let rec print_hund x =
let (ans,tosub) = (  
	if x < 500 then
	(if x < 200 then
...
let pi2 = 2.0 *. pi in
let rec getbigger a  p =
    if a < p then p
    else getbigger a (p*. 2.0) in
let rec minusp a p =
    if a <= pi2 then a
    else 
	(if a >= p then minusp (a -.p) (p/.2.0)
         else minusp a (p/.2.0)) in
let rec reduction_2pi a = 
    let p = getbigger a pi2 in
    minusp a p in 
let rec kernel_sin a = 
a -. 0.16666668*.a*.a*.a +.0.008332824*.a*.a*.a*.a*.a -.0.00019587841 *.a*.a*.a*.a*.a*.a*.a in
let rec kernel_cos a =
1.0 -. 0.5*.a*.a +. 0.04166368*.a*.a*.a*.a -. 0.0013695068*.a*.a*.a*.a*.a*.a in
let rec flag a =
    if a > 0.0 then 1.0 else -1.0 in
let rec sin a =
    let f = flag a in
    let a = fabs a in 
    let a = reduction_2pi a in 
    let a2 = (if a >= pi then a -. pi else a) in
    let f = (if a >= pi then -1.0*.f else f )in
    let a = (if a2 < (pi/. 2.0) then a2 else pi -. a2 )in
    let a = (if a <= (pi/.4.0) then (kernel_sin a) else kernel_cos(pi/.2.0 -. a) )in
    f *. a in
let rec cos a =
    let f = 1.0 in
    let a = fabs a in
    ...
    let f = (if a >= pi then -1.0*.f else f )in
    let a = (if a2 < (pi/. 2.0) then a2 else pi -. a2 )in
    let a = (if a <= (pi/.4.0) then (kernel_cos a) else kernel_sin(pi/.2.0 -. a) )in
    f *. a in
(*
...
let rec write_ppm_header _ =
  (
    ...
    print_char 10;
    print_hund image_size.(0);
    print_char 32;
    print_hund image_size.(1);
    print_char 32;
    print_hund 255;
    print_char 10
  )
