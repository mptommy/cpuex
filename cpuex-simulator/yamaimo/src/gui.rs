...
pub fn build_ui(application: &gtk::Application,core:&EnvBase) {
    ...
    let model = Rc::new(create_model(core));
...
fn create_model(core:&EnvBase) -> gtk::ListStore {
    ...
    let mut data2 :Vec<DataI>=Vec::new();
   // let inst_data = core.fetch_memory ();
      
  //  let inst_decode = core.decode_inst(&inst_data);
    let mut base_addr = 0;
    
    while true{
        let fetch_data = ((core.m_memory[base_addr as usize + 3] as i32) << 24) |
        ((core.m_memory[base_addr as usize + 2] as i32) << 16) |
        ((core.m_memory[base_addr as usize + 1] as i32) <<  8) |
        ((core.m_memory[base_addr as usize + 0] as i32 )<<  0);
        let inst_decode = core.decode_inst(&fetch_data);
        if fetch_data == 0{break;}
        data2.push(
            DataI{
                number: fetch_data
            }
  ...
  /*  let data: [Data; 14] = [
    ...
    for (d_idx, d) in data2.iter().enumerate() {
