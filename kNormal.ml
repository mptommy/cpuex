...
let rec fv = function (* 式に出現する（自由な）変数 (caml2html: knormal_fv) *)
  ...
  | Add(x, y) | Sub(x, y) | FAdd(x, y) | FSub(x, y) | FMul(x, y) | FDiv(x, y) | Get(x, y) -> S.of_list [x; y] (* x, yが要素の集合を返す *)
...
let insert_let (e, t) k = (* letを挿入する補助関数。式eを受け取り、新しい変数xを作ってlet x = e in ...という式を返す。（ただしeが最初から変数の時は、それをxとして利用し、letは挿入しない）kはinの中を作るための関数。 (caml2html: knormal_insert) *)
  match e with
  | Var(x) -> k x
  | _ ->
...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax.If(Syntax.Eq(e1, e2), e3, e4) -> (* Ifの本質 *)
      insert_let (g env e1)
        (fun x -> insert_let (g env e2)
            (fun y ->
